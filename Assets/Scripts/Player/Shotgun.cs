...
public class Shotgun : MonoBehaviour
{
    ...
    private IEnumerator ShotFX()
    {
        ...
        weaponModelTransform.DOPunchPosition(new Vector3(0, modelRecoilY, modelRecoilZ), 0.6f);
        if (weaponModelTransform.localRotation.x < -1)
        {
            weaponModelTransform.DOLocalRotate(Vector3.zero, 0.2f);
        }
        else
        {
            weaponModelTransform.DOBlendablePunchRotation(new Vector3(ModelRotationRecoil, 0, 0), 0.5f);
        ...
        gunShotPos[0].position, gunShotPos[0].rotation);
    ...
    private IEnumerator LineFX()
    {
        ...
        for (int i = 0; i < gunShotPos.Length; i++)
        {
            rastroTiro.SetPosition(0, gunShotPos[i].position);
